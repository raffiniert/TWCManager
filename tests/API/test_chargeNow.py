...
print(
    "Using values First: "
    + str(values["targetFirst"])
    + " and Second: "
    + str(values["targetSecond"])
    + " for chargeNow rate testing."
)
...
data = {"chargeNowDuration": 3600, "chargeNowRate": -10}

values["tests"]["chargeNowNegativeRate"] = {}
values["expected"]["chargeNowNegativeRate"] = 400
try:
    response = session.post(
        "http://127.0.0.1:8088/api/chargeNow", json=data, timeout=30
    )
...
data = {"chargeNowDuration": -3600, "chargeNowRate": 24}
...
data = {"chargeNowDuration": 3600, "chargeNowRate": int(values.get("targetFirst", 0))}
...
try:
    response = session.post(
        "http://127.0.0.1:8088/api/chargeNow", json=data_ascii, timeout=30
    )
...
data = {"chargeNowDuration": 3600, "chargeNowRate": int(values.get("targetSecond", 0))}
...
if int(values["targetFirst"]) != float(
    values["status"]["First"]["maxAmpsToDivideAmongSlaves"]
):
    print(
        "Error: maxAmpsToDivideAmongSlaves doesn't match target for first chargeNow test"
    )
    values["tests"]["chargeNowFirst"]["fail"] = 1

if int(values["targetSecond"]) != float(
    values["status"]["Second"]["maxAmpsToDivideAmongSlaves"]
):
    print(
        "Error: maxAmpsToDivideAmongSlaves doesn't match target for second chargeNow test"
    )
    values["tests"]["chargeNowSecond"]["fail"] = 1
